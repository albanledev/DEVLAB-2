...
let dataArray = [response.data];  // Convert response.data to an array
    ...
    dataArray.forEach(element => {
        containerMovie.innerHTML =     "<h1 class='my-[30px] mx-[20px] font-poppins font-semibold text-[20px]'>" + element.title + '</h1>' + `<div class='sm:flex'><img class="w-[200px] my-[40px] object-cover rounded-[9px] mx-[20px] sm:w-[500px]" src="https://image.tmdb.org/t/p/w500${element['backdrop_path']}" alt=''>` +

        '<p class="flex items-center mx-[20px]">'+element.overview+'</p></div>'
